...
## 2026-09-05 — N72R8R1 protocol correction and N72R9 reservation

- **Protocol correction:** N72R8's old confirmation was reclassified as ordinary target-session diagnostic evidence. Its raw-binding switch fields and true public-assignment crossings are now separated; the old `EVIDENCE_BACKED_EXHAUSTION_OF_REGISTERED_N72R7_ROUTES` terminal wording is not carried forward. The original N72R8 report/gate and all numerical evidence remain read-only under `outputs/N72R8R1/attempts/` and the historical N72R7 tree.
- **Reservation audit:** A metadata-only N72R9 touched-sequence registry scanned the available 65 train/val sequences using historical sequence references, protocol manifests, source lists and legacy evaluation path evidence. The first six scanner attempts are retained for environment/result-tree enumeration failures; the repaired scan completed without reading annotations or outcome values. All `65/65` sequences have conservative historical-touch evidence, so `eligible=0` and no fresh confirmation sequence was fabricated or reserved. `outputs/N72R9/confirmation_reservation/reservation_manifest.json` is sealed with registry hash `f1e3438ea2245844e34a2dfdddbcbd226d1408d13276fb9bfaa652b9cc952b55` and protocol hash `d9601fa6c7067af233c5d1c03bfcf262fa26decfa61d24dce69380901530b4be`.
- **Current state:** N72R9 may continue on explicitly non-fresh development data for source-aware temporal training and mechanism replay, but it cannot claim an untouched confirmation. No N72R9 training, replay, calibration, selector, LoRA or production promotion has started at this entry; all interactions remain `simulated_from_gt`, and runtime future GT remains forbidden.

## 2026-09-05 — N72R9 source-aware temporal training and future-requery replay

- **Training/corpus:** After the sealed zero-eligible confirmation reservation, N72R9 froze a 32-event/18-sequence development protocol (30 train events, 2 validation events; action counts ADD 4, AUTHORITATIVE_REASSIGN 14, ATOMIC_ID_SWAP 3, RECOVER 11). The source-aware corpus contains 3,000 train and 200 validation examples. An actual source-aware temporal candidate-set model was trained on CPU; validation-loss checkpoint selection chose epoch 5 (`val loss=0.4806792`). Validation target-candidate accuracy was `0.9037`, but NONE accuracy was `0/13`. The checkpoint is isolated and not production-authorized.
- **Runtime/replay:** A 3-event smoke and full 32-event paired replay passed. Each event used one independent process and three variants: frozen B0, B0+current target-session candidates+temporal model, and the same model with future-frame requery candidates added only after runtime uncertainty. The run sealed all runtime JSONL before posthoc GT scoring. It produced `9,696` runtime frame rows and `80,207` candidate rows; duplicate/missing/frame-axis/matrix/mapping/runtime-GT audit errors were all zero.
- **Causal result:** The combined `TEMPORAL_CURRENT-B0` development path showed identity-error reduction `0.0978793/0.0521236/0.0373802` at H20/H50/H100 with sequence-cluster CI lower bounds `0.0490741/0.0219580/0.0138136`, but had 9 protected identity regressions at H20. The isolated `TEMPORAL_REQUERY-TEMPORAL_CURRENT` increment was `0/-0.0006435/0` with CI lower bounds `0/-0.0008333/-0.0004167`; it did not establish future-requery benefit. Requery triggered 591 times and applied 375 times, with 31 target assignment changes and 42 raw binding switches; public ID changed zero times. The required complete new-candidate-selection milestone count is zero; one partial wrong-to-correct case is retained and documented.
- **Gate/decision:** `outputs/N72R9/n72r9_final_gate.json` records `N72R9_DEVELOPMENT_COMPLETE_NO_FRESH_CONFIRMATION` and `research_gate=FAIL_FUTURE_REQUERY_EFFECT`. The result is not a PASS, does not authorize calibration/selector/decoder LoRA/production, and does not claim real-human evidence. The minimum next step is a genuinely untouched independent sequence reservation (and, for a real-user claim, external provenance-complete human tape); no local eligible sequence currently exists.
- **Failures retained:** N72R9 retains the corpus compile/authority-axis fixes, default-interpreter torch failure, training-smoke shape failure, and first aggregate `NameError` under `outputs/N72R9/**/attempts/`; each was minimally repaired and rerun on the same input. No N36–N72R8 artifact, third-party SAM3 file, shared checkpoint or production MOT/OVMOT configuration was modified.

## 2026-09-05 — N72R10 Stage 00 mechanism-label correction

- **Correction:** The N72R9 source named `TARGET_SESSION_REQUERY` is frozen N72R7 R5 static event requery reuse, not a fresh current-frame SAM3 session. It is reclassified as `STATIC_EVENT_REQUERY`; `FUTURE_FRAME_REQUERY` is reserved for the new N72R10 session and is zero in N72R9 training/validation.
- **Preservation:** N72R9 reports, gate, replay, checkpoint and output tree were not rewritten or rerun. The historical research gate remains `FAIL_FUTURE_REQUERY_EFFECT`, all interactions remain `simulated_from_gt`, and no downstream authorization was granted.
- **Next:** Run the CPU-only audit of the nine N72R9 ATOMIC protected regressions before implementing any shared atomic pair repair.

## 2026-09-05 — N72R10 true future-frame closed-loop reacquisition

- **Hypothesis and scope:** N72R10 separated N72R9's frozen/static event re-query from a genuinely fresh current-frame SAM3 session. The new path starts after an uncertainty trigger, queries the causal current frame in an event-local session, optionally propagates only the selected source, changes raw/native trajectory binding while preserving immutable public identity, and exposes the first effect at `t+1`. N36–N72R9 evidence, the checkpoint, candidate definition, Hungarian solver, metrics and `third_party/sam3` were preserved.
- **Structural result:** The true re-query batch completed `32/32` events over `18` sequences and `3,200` future frames. It produced `3,020` future candidate rows/nonempty frames with zero missing frames, duplicate candidate UIDs, mapping failures, public-ID changes, event-frame memory reads, future-boundary violations or unclosed backends. The official runtime used video/output CPU offload and four isolated workers on GPUs `1,2,3,4`; state CPU offload was not enabled. All interactions remain `simulated_from_gt`, and runtime future GT was false.
- **Repair evidence:** The first two runs of one official observation failure are retained (`positive-area box` and `non-empty mask with zero-area box`). A deterministic box derived only from the same official non-empty mask repaired that row without replacing the raw box; the third attempt passed. A replay provenance failure due to missing `started_at_utc` is also retained and was repaired before the complete attempt-2 replay.
- **Training:** A source-aware temporal candidate-set model was actually trained in an isolated checkpoint. The sealed corpus had `3,000` train and `200` validation examples (`30/2` events, `16/2` sequences), with `2,833`/`187` future-requery rows. The frozen event pool can produce at most `3,200` causal future examples; no lawful local expansion was found. Validation contained zero positive `FUTURE_FRAME_REQUERY` labels and only `13` NONE examples, so the checkpoint is a development probe, not selector or production evidence.
- **Paired replay:** The complete attempt-2 E0/E1/E2 replay passed `32/32` events, `9,696` runtime frame rows and `80,207` candidate rows with zero audit errors. E1 retained positive aggregate H20/H50/H100 reductions (`0.08809/0.03925/0.02875`, all lower CIs positive). The isolated E2−E1 increment was positive with lower CIs `0.001634/0.001111/0.00000567` at H20/H50/H100, but the combined E1 path had protected regressions `7/7/7`, and the E2 increment had `2/3/6` protected regressions. Action-level lower CIs were often zero or negative.
- **Closed-loop mechanism:** The runtime had `591` triggers and applied fresh sources, `33` frozen-model fresh selections, `29` target-quality selections, `12` target-public assignments, `4` wrong selections/assignments, and `21` target-quality solver refusals. One event completed the full trigger→fresh candidate→selection→assignment→raw/native rebinding→immutable public ID→posthoc wrong-to-correct milestone. This confirms the path can execute, while the 21 solver refusals identify global association competition as the primary next diagnostic rather than proof of a general solution.
- **Decision:** The development gate remains `FAIL_N72R10_DEVELOPMENT_GATE` / `FAIL_FUTURE_EFFECT_OR_DEVELOPMENT_READINESS`. The reasons are nonzero protected regression, zero positive future-requery labels in validation, and the preregistered training-size shortfall. Calibration, selector, decoder LoRA and production promotion remain unauthorized. The minimum next step is a larger lawful same-run public-authority interaction pool with sequence-disjoint validation positives, followed by an isolated target-edge solver bridge test; historical static rows and duplicated/horizon-extended events must not be reused.
- **Evidence:** Final machine gate is `outputs/N72R10/stage_09_gate.json`; milestone audit is `outputs/N72R10/true_requery_milestone_audit.json`; root-cause classification is `outputs/N72R10/stage_11_root_cause_classification.json`; distribution audit is `outputs/N72R10/stage_10_training_distribution_audit.json`; final narrative is `docs/N72R10_FINAL_REPORT.md`.

## 2026-09-06 — N72R11 on-demand requery and target-edge bridge

- **Scope and preservation:** N72R11 implemented an isolated on-demand future-frame requery controller, an assignment-aware target-edge bridge and a Temporal V3 model definition. N36--N72R10 evidence, frozen checkpoint/candidate/metric definitions and `third_party/sam3` were preserved. All events remain `simulated_from_gt`; real-human evidence remains `0`.
- **Structural evidence:** The controller's focused toy contract passed, including current-frame memory-read exclusion, first visibility at event+1, mapping/GT audit fields and release of a selected requery session after observation materialization. A short E0/E1/E2 runtime wiring smoke sealed successfully but was explicitly non-scientific because its horizon was shorter than H100. The protected audit retained seven historical regressions and classified the root cause as `TARGET_EDGE_OVER_STEALS_PROTECTED_INCUMBENT`.
- **Secondary execution:** The frozen N72R11 schedule had 527 event-policy instances across 18 sequences (`428` train, `99` validation). The first formal batch produced `457 PASS + 70 failures` (37 OOM, 33 invalid-box validation failures). A fresh-process/fresh-GPU retry with expandable allocator repaired all 33 invalid-box keys and left 37 OOM failures. The lossless merge contains 527 unique keys, 490 PASS and 37 `FAIL_CHILD`; duplicate and missing key counts are zero.
- **Final OOM repair:** The final allowed equivalent memory repair closed the selected `FutureFrameRequerySession` after materializing observation/audit rows, deleted pending references, ran `gc.collect()` and `torch.cuda.empty_cache()`, while retaining official continuation state needed for propagation. The same full-window event-policy smoke still failed inside official SAM3 segmentation while requesting 1.19 GiB with 1.05 GiB free. The complete traceback and all earlier attempts remain retained.
- **Gate/decision:** The causal corpus builder correctly rejected the partial merge. Temporal V3 training, bridge training, formal E0/E1/E2 replay, paired scoring and production authorization were not run. N72R11 is `BLOCKED_N72R11_SECONDARY_OOM`, not PASS. The only lawful next step is rerunning the 37 preserved OOM keys on a genuinely less-contended/larger-memory environment or an already-supported official memory mechanism without changing the frozen protocol.
- **Evidence:** Final gate is `outputs/N72R11/n72r11_final_gate.json`; controller status is `outputs/N72R11/CONTROLLER_STATUS.json`; final report is `docs/N72R11_FINAL_REPORT.md`; method audit is `docs/N72R11_METHOD_AUDIT.md`.

## 2026-09-06 — N72R11R1 serialized OOM recovery and terminal resource block

- **Scope and preservation:** N72R11R1 created an isolated recovery branch and kept the N72R11 527-key schedule, checkpoint, candidate stream, metrics, historical outputs and `third_party/sam3` read-only. All interaction records remain `simulated_from_gt`; real-human evidence remains `0`.
- **Engineering repair:** The batch supervisor now queries live physical GPU telemetry and compute-app support, chooses idle/free GPUs dynamically over all discovered devices, caps concurrency at four, records launch snapshots and preserves physical-GPU identity while children use logical `cuda:0`. The generator uses the real official `official_batched_grounding_batch_size=1` interface. SAM3 geometry/propagation and OSNet materialization are serialized in independent processes; the controller releases completed SAM3 sessions before post-session feature extraction. Official video/output CPU offload was active; official state CPU offload was not available/claimed. Allocator setting was `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`.
- **Smoke:** The required full-window key `n72r5-pool-n37-dancetrack0001-0296-authoritative_reassign-001:secondary:060` passed once with 41 frames and no horizon override. Its peak telemetry reached `36,109 MiB` allocated and `36,440 MiB` reserved. A post-close audit provenance field was corrected after the smoke without rerunning it; targeted controller regression passed, and later retry PASS artifacts carry the corrected field.
- **Recovery:** The exact 37-key OOM manifest was derived from immutable evidence. The smoke counted as one success, so the remaining 36 keys were run by one dynamic supervisor. Attempt 05 completed `25 PASS + 11 OutOfMemoryError`; all 36 launches were on idle GPUs, with zero same-physical-GPU overlap and zero scheduler snapshot failures. The sealed R1 retry manifest has `26 PASS` including smoke and `11` OOM failures.
- **Strict gate:** Existing merge produced `527` unique records, `516 PASS`, `11 FAIL_CHILD`, duplicate `0`, missing `0`, and `PASS_ALL_SELECTED=false`. The 11 failure artifacts contain full traceback/log evidence and report 2--82 MiB allocation requests while only 1.12--73.12 MiB remained free on 39.39 GiB devices. This is intrinsic official SAM3 future-propagation memory exhaustion after the authorized serialization/batch-size/allocator/dynamic-scheduler repair, not external contention or metric failure.
- **Downstream decision:** The causal corpus, Temporal V3, TargetEdgeBridge, formal E0/E1/E2 replay and production promotion were not run because the 527/527 gate failed. N72R11R1 terminates as `BLOCKED_INTRINSIC_OFFICIAL_SAM3_MEMORY`, not a research PASS or research failure. The only lawful next step is a genuinely larger/less-contended environment or an already-supported official state-memory mechanism, rerunning only the 11 preserved keys with the frozen protocol.
- **Evidence:** Terminal gate `outputs/N72R11R1/n72r11r1_final_gate.json`; controller status `outputs/N72R11R1/CONTROLLER_STATUS.json`; retry audit `outputs/N72R11R1/retry_output_audit_attempt_01.json`; final report `docs/N72R11R1_FINAL_REPORT.md`.

## 2026-09-07 — N72R11R2 official streaming, resource-censored development and terminal block

- **Hypothesis/protocol:** N72R11R2 tested whether official SAM3 future propagation could be streamed with memory release while preserving the frozen N72R11/N72R9 candidate, public-ID, Hungarian, H20/H50/H100 and posthoc-GT protocol. The pinned checkpoint and `third_party/sam3` were preserved; every runtime record remained `runtime_future_gt_used=false` and `simulated_from_gt`.
- **Streaming evidence:** The first two official trim attempts retained `KeyError: multistep_point_inputs`. A minimal adapter delayed trim until a compatible output, and attempt 3 passed scientific candidate equivalence on 41/41 frames with zero scientific/mask mismatches. The trim was requested but effective count remained zero (34 schema blockers); verified output/video CPU offload was active, state offload was not. A singleton 1-object fallback was rejected because the fixed 16-object checkpoint could not load into that shape.
- **Resource result:** The 527-key schedule was losslessly audited: 527 observed/unique, zero duplicate/missing/invalid, 516 executable and 11 residual official CUDA-OOM records. The 11 were retained and resource-censored, not marked PASS. Formal E0/E1/E2 development replay completed 31/32 required events; one `dancetrack0062` ADD event failed inside official future propagation with a 2 MiB allocation request and 1.06 MiB free. Attempts 1 and 2 supervisor interruptions and attempt 3 partial completion remain preserved.
- **Training:** The repaired resource-censored causal corpus passed with 516 events, 24,690 train examples and 5,700 validation examples on a fixed 12/6 sequence split. Isolated Temporal V3 training and TargetEdgeBridge training both completed, but V3 readiness failed: future-positive accuracy `0.451932 < 0.60`, NONE accuracy `0.265533 < 0.50`, and target-candidate readiness was undercovered. No production promotion was granted.
- **Descriptive effect:** The corrected 31-event aggregate is descriptive only. E2−E0 identity-error reduction was `+0.035413/-0.096410/-0.180528` at H20/H50/H100 with bootstrap lower bounds `-0.138979/-0.275483/-0.345926`; assignment changes were 371/892/1885 with 105/84, 190/335 and 285/832 correct/incorrect crossings. The isolated E2−E1 lower bounds were zero at all horizons. These results do not satisfy a complete future-effect gate.
- **Decision:** N72R11R2 is `BLOCKED_N72R11R2_INCOMPLETE_FORMAL_REPLAY_AND_V3_NOT_READY`; production, Oracle, calibration, selector and decoder LoRA remain unauthorized. The minimum next step is only a larger/less-contended memory environment or a verified official state-memory mechanism for the preserved failed event; no window shortening, precision/checkpoint change or blind downstream training is justified. Final machine gate: `outputs/N72R11R2/stage_15_final_gate.json`; final report: `docs/N72R11R2_FINAL_REPORT.md`.

## 2026-09-08 — N72R11R3 state alignment, V3 generalization failure and E0/E1 closure

- **Scope and repairs:** N72R11R3 preserved N36–N72R11R2 evidence, checkpoint/candidate/metric definitions and `third_party/sam3`. It introduced a shared `TemporalIdentityState`/8-D causal feature policy across corpus construction, V3 rollout and runtime; safe trusted/distractor admission; a shared explicit 14-D TargetEdgeBridge feature builder; corrected target-vs-best-other Bridge loss; and corrected V3 readiness source semantics. No production path was authorized.
- **Resource/contract evidence:** The CPU contract and static shared-state audit passed. The original formal OOM event completed its one permitted full-window resource check without OOM; no 516-event SAM3 corpus was regenerated. The corrected bootstrap corpus retained `516` events (`24,690` train / `5,700` validation) and the on-policy corpus completed via two sequential CPU split workers (`24,690` / `5,700` rows) with runtime future GT false.
- **Failure preservation:** The first on-policy attempt retained a zero-norm missing-feature traceback. Attempt 2 was killed with exit `137` by host global OOM (about `6.4 GiB` available, no swap); the process-level record and kernel diagnosis remain under `outputs/N72R11R3/onpolicy_corpus_attempt_02/attempts/`. Passing zero-filled decoder tokens as absent state features (`None`) and split workers repaired the path. A self-rollout caller exposed the same missing-feature issue once; its failure and one-event regression are retained. The first aggregator rejected 32 valid events because `done.json` omitted a seal-projected `runtime_gt_read`; validator repair plus same-manifest reaggregation passed `32/32`.
- **Training/readiness:** Corrected bootstrap V3 used the frozen two epochs. Ordinary one-epoch finetune and the single train-only category-balanced repair both completed but failed the corrected readiness gate. The fixed validation-loss rule retained bootstrap as final V3 (`target candidate 0.511730`, `NONE 0.216716`, future-requery `0.642857` on only `42` labels).
- **Development effect:** Because V3 was not ready, only E0/E1 development replay was allowed. All `32/32` frozen events over `18` independent sequences passed. E1−E0 pooled identity-error reduction was `-0.019576/-0.081081/-0.068690` at H20/H50/H100; sequence-cluster 95% CI lower bounds were `-0.106251/-0.157491/-0.121157`. Assignment changes were frequent but incorrect crossings exceeded correct crossings (`64/76`, `92/218`, `176/391`), and protected regressions were `8/19/25`. E2/E3 were not run, so Bridge/live effects remain null.
- **Decision:** State alignment is structurally repaired but V3 generalization is not. N72R11R3 is `FAIL_V3_GENERALIZATION_AFTER_STATE_ALIGNMENT`, with main direction `REVISE`: retain persistent public identity/exact global assignment foundations, revise V3 candidate/base supervision and obtain provenance-complete real-human evidence before any Bridge/live or production work. Final gate: `outputs/N72R11R3/n72r11r3_final_gate.json`; controller: `outputs/N72R11R3/CONTROLLER_STATUS.json`; report: `docs/N72R11R3_FINAL_REPORT.md`.

## 2026-09-09 — N72R11R4 exact solver-on-policy and PCTIS closure

- **Hypothesis/protocol:** N72R11R3's stale candidate/neighbor/motion features and scorer UID could have invalidated its V3 result. N72R11R4 froze the 32-event/18-sequence N72R9 development protocol, rebuilt exact solver-on-policy features, then tested same-capacity public-competition features. Checkpoint, candidate stream, Hungarian solver, horizons and GT boundary were unchanged; all events remain `simulated_from_gt` and real human tape remains `0`.
- **Implementation/training:** The exact V3 corpus passed its contract with `24,690` train and `5,700` validation examples. PCTIS added exactly six competition features (541-D input), copied the first 535 V3 columns, zero-initialized the six new columns, and passed initialization smoke. The prescribed initial two-epoch train and one-epoch exact on-policy finetune both ran in isolated checkpoints. Readiness failed for target `0.50046<0.70`, NONE `0.25592<0.50`, and positive boundary `0.62351<0.70`; protected-safe was `0.99847`.
- **Effect:** Complete E1A/E1B replays passed 32/32 events. E1A identity-error reductions were `-0.019576/-0.081081/-0.068690` at H20/H50/H100; E1B was `+0.042414/-0.000644/-0.008307`. E1B improved over E1A by `+0.061990/+0.080438/+0.060383`, and harmful crossings/protected regressions decreased, but every E1B sequence-cluster CI lower bound remained below zero. RECOVER H20 remained unfavorable (`18/26` correct/incorrect), so this is descriptive mechanism evidence, not a strict pass.
- **E2/resource failures:** Initial E2 had `25 PASS + 7 FAIL_CHILD`; fresh retry recovered six, leaving `dancetrack0002:0329 AUTHORITATIVE_REASSIGN`. The final equivalent outer-GPU1 retry still failed inside official SAM3 with a 42 MiB allocation and 35 MiB free; its artifact reported internal GPU0 allocation. Corrected audit classifies the six initial `-9` records as SIGKILL/resource-or-external-kill without process traceback and records three OOM records across attempts. Lossless merge is `32` unique records, `31 PASS`, `1 unresolved`, duplicate/missing `0`; E2 aggregate and Oracle were correctly withheld.
- **Decision:** N72R11R4 is `BLOCKED_RESOURCE_INCOMPLETE_E2_FAIL_FUTURE_EFFECT`, disposition `REVISE`. Keep exact public assignment, causal event+1 boundary and PCTIS code as isolated research artifacts; do not promote PCTIS, run Oracle, train selector/calibration/LoRA, or claim production. The only minimal technical next step is to verify/fix official SAM3 internal device placement or obtain a genuinely supported larger-memory/state-streaming environment and rerun the single frozen event. All failures and stage gates are under `outputs/N72R11R4/`; report: `docs/N72R11R4_FINAL_REPORT.md`; code pushed at commit `f1bc28e6d52b0b01462f52a78c04f414d7c85d78`.

## 2026-09-09 — N72R11R5 interaction-window TrackEval diagnostic

- **Protocol and preservation:** N72R11R5 was implemented as post-hoc evaluation over the frozen N72R11R4 E0/E1A/E1B sealed runtime rows. It uses exactly `[event_frame+1, event_frame+H]` pseudo sequences for H20/H50/H100, official pinned TrackEval commit `12c8791b303e0a0b50f753af204249e622d0281a`, all 32 events and 18 independent sequences, with an expected 288 per-event variant records. No SAM3 rerun, retraining, E2 evaluation, GT-ID remapping, threshold/checkpoint change, or modification of N36–N72R11R4/third-party evidence occurred. The events remain `simulated_from_gt`; real-human tape remains `0`.
- **Smoke and compatibility repairs:** The one-event exporter smoke passed with 9 window records. The first exporter smoke failed before experiment logic because the direct script lacked the project-root import path; that failure is retained. TrackEval smoke attempts 1 and 2 exposed two scalar-argument parsing bugs in the pinned old CLI (`SEQMAP_FILE` and `OUTPUT_FOLDER` were parsed as one-element lists); both failures are retained. A narrow runner-side compatibility unwrap plus the supported `SEQ_INFO` route made the same H20 smoke pass with 3/3 records. The TrackEval submodule remained byte-identical.
- **Actionable root cause:** The first full export stopped at event `n72r5-pool-n37-dancetrack0027-0148-authoritative_reassign-007`, absolute frame `222` / relative frame `74`, public ID `1007`, where the exact solver row is explicitly assigned but has box `[571.0000228881836,307.00000047683716,571.0000228881836,308.00000050105155]` (`x2==x1`). A lossless full preflight then enumerated **42** invalid assigned logical rows across exactly two events: **36** in `dancetrack0027` and **6** in `dancetrack0033`; each of the three logical variants has 14. The rows have no alternate geometry field. Clipping, dropping, replacement boxes, or candidate substitution would change the sealed solver output, so no post-hoc repair is legal.
- **Gate/decision:** Export is `BLOCKED_INVALID_SEALED_ASSIGNED_BOXES`; TrackEval full matrix was not run, aggregation refused the input, and the final gate is `BLOCKED_INCOMPLETE_WINDOW_TRACKEVAL` with `0/288` valid records and `metrics_available=false`. The H20 smoke is not promoted to a full result. Q1–Q3 are not estimable from this blocked matrix; the earlier N72R11R4 `+0.042414` remains a frozen legacy identity-error value, not an interaction-window TrackEval result. No calibration, selector, decoder LoRA, or production authorization was granted.
- **Evidence and next step:** Machine gate is `outputs/N72R11R5/FINAL_STATUS.json`, geometry evidence is `outputs/N72R11R5/export_failure_invalid_assigned_boxes.json`, stage gates are under `outputs/N72R11R5/stage_01_status.json`–`stage_03_status.json`, and the final report is `docs/N72R11R5_WINDOW_TRACKEVAL_REPORT.md`. The minimum lawful recovery is upstream re-sealing/regeneration of the two affected runtime windows with finite positive exact-solver boxes, followed by the same lossless preflight; N72R11R5 must not repair or omit them. The hardened exporter and strict aggregation/finalizer were pushed at commit `62d95ce` on branch `codex/n72r11r5-window-trackeval`.

## 2026-09-09 — N72R11R5R1 positive-geometry repair and interaction-window TrackEval closure

- **Scope/protocol:** N72R11R5R1 kept the frozen N72R9 32-event/18-sequence protocol, N72R11R4 E0/E1A/E1B sealed streams, checkpoints, candidate order/definition, Hungarian solver, H20/H50/H100 windows and posthoc metrics unchanged. All events remain `simulated_from_gt`; real-human tape remains `0`. N36–N72R11R5 evidence and `third_party/sam3`/TrackEval were not rewritten.
- **Root cause and repair:** The historical N72R11R5 preflight retained 42 non-positive assigned-box rows in two events. A source-level audit found 40 invalid candidates among 57,987 source rows (`c0=14`, `c1=14`, `requery=10`, `target_stream=2`) and zero empty-after-filter solver frames. The opt-in `require_positive_geometry` policy now rejects invalid candidates before model/solver with bounded provenance, preserves source-axis indices, and regenerates E0 from frozen sources. The original 42-row failure and the first validation-source exporter failure remain preserved.
- **Structural execution:** The required dancetrack0027 H100 smoke passed for E1A/E1B with positive finite assigned geometry, runtime GT violations `0`, and identical corrected E0 hashes. Corrected formal E1A and E1B replay each completed `32/32` unique events with zero missing/duplicate/validator failures; each treatment rejected 30 invalid source candidates before the solver. The evidence-backed resolver mapped the two protocol `validation` labels to frozen `train` GT sources only after exact N72R6 hashes and frozen N72R7 manifests matched.
- **Effect:** Reused causal aggregation remains `FAIL_FUTURE_EFFECT`. E1A H20/H50/H100 identity-error reductions were `-0.019576/-0.081081/-0.068371`, with lower CIs `-0.106251/-0.157491/-0.120793`; E1B reductions were `+0.042414/-0.000644/-0.008307`, with lower CIs `-0.041032/-0.066751/-0.046482`. The corrected-vs-old comparison shows no new causal signal; only a one-row E1A H100 crossing/tiny numerical difference changed.
- **TrackEval:** The corrected exporter created 96 pseudo-sequences and `288/288` unique interaction-window records. Pinned TrackEval commit `12c8791b303e0a0b50f753af204249e622d0281a` completed H20/H50/H100 (`96` records each), and aggregation passed. These are interaction-window diagnostics, not official DanceTrack benchmark scores; E2 was not run.
- **Decision:** Structural status is `PASS_N72R11R5R1_WINDOW_TRACKEVAL`, while the research effect gate remains `FAIL_FUTURE_EFFECT`. Calibration, selector, decoder LoRA, E2 and production promotion remain unauthorized. The next defensible step is a newly frozen association/interface or provenance-complete real-human tape; do not bypass the failed effect gate with threshold/checkpoint/LoRA changes. Final report: `docs/N72R11R5R1_GEOMETRY_CORRECTED_TRACKEVAL_REPORT.md`.

## 2026-09-10 — N72R12 counterfactual safe intervention and learned-safe gate

- **Hypothesis/protocol:** N72R12 tested whether PCTIS was useful but unsafe because it changed the global assignment when baseline should have been retained. The frozen N72R9 protocol (`32` events, `18` independent sequences), corrected N72R11R5R1 E0/E1B streams, PCTIS checkpoint, candidate axis, exact solver, H20/H50/H100 windows and post-hoc metrics were reused. Runtime never read GT; all interactions remain `simulated_from_gt`, and real-human evidence remains `0`.
- **CSI implementation:** Added `COUNTERFACTUAL_SAFE_V1`: compute base and PCTIS proposal assignments on the same candidate axis, then APPLY only for an accepted selected candidate with target assignment change, target ownership/geometry validity, no non-target public collateral change, strict target-edge dominance and positive edge gain. Otherwise commit the base solver and prevent rejected proposals from updating memory. E1B remained the unconditional control. Existing candidate/public/native mapping, Hungarian solver and admission thresholds were not changed.
- **Oracle headroom:** The post-hoc Oracle was completed and marked runtime-ineligible. It provides headroom only, not a method or scientific runtime result; it cannot authorize production.
- **Deterministic execution:** The targeted H100 smoke and formal `32/32` event runtime passed. The sealed CSI stream covered `3,200` future frames: `290 APPLY`, `2,910 KEEP`, with zero formal runtime failures, duplicate/missing events or runtime-GT flags. The deterministic causal H20 reduction was `+0.040783`, but its sequence-cluster CI was `[-0.041032,+0.193767]`; H50/H100 reductions were `-0.000644/-0.008307`. Protected regressions were `7/8/14`. Interaction-window TrackEval completed `288/288` records using pinned TrackEval `12c8791b303e0a0b50f753af204249e622d0281a`; E1C versus E0 at H100 was HOTA `-0.005353`, AssA `-0.002686`, IDF1 `-0.007961`, IDSW `+45`. The deterministic strict gate is false.
- **Intervention confusion:** A post-hoc H100 audit on visible target frames classified `153` harmful E1B proposals and `105` beneficial proposals. CSI blocked `45/153` harmful proposals and retained `21/105` beneficial proposals; visible-frame intervention precision was `0.072664`, harm-block rate `0.294118`, and benefit-retention rate `0.20`. These are diagnostic counts, not training labels and not a claim of causal success.
- **Learned gate:** Because deterministic CSI did not pass, the one authorized small gate was trained from the frozen resource-censored N72R11R4 corpus. It used `24,202` train and `5,688` validation examples, a fixed 12/6 sequence split, 37-D causal features, 10 epochs, batch 256, AdamW, learning rate `1e-3`, weight decay `1e-4`, BCEWithLogitsLoss, train-only capped positive weight `10`, seed `7212`, and threshold `0.5`; checkpoint selection used minimum validation BCE only. The actual training and save/restore smoke passed, but validation readiness failed: precision `0.020325`, recall `0.112360`, predicted APPLY `492`, unsafe APPLY rate `0.979675`.
- **Learned execution/effect:** E1D formal runtime passed `32/32` events and `3,232` frames; learned APPLY was `109`, final CSI∩learned APPLY was `83`, and `26` learned proposals were blocked by the deterministic safety ceiling. Learned causal E1D−E0 reductions were `+0.052202/+0.010296/+0.009904` at H20/H50/H100, but protected regressions remained `7/8/14` and H20 CI was `[+0.000827,+0.191221]`. Learned TrackEval completed `384/384` records; at H100 E1D−E0 was HOTA `-0.004852`, AssA `-0.003130`, IDF1 `-0.005311`, IDSW `+47`. The learned strict gate is false.
- **Failures retained:** The N72R12 corpus axis/metadata failures, training alignment failure, E1D checkpoint-parameter and short-horizon smoke failures, learned exporter `NameError`, TrackEval wrapper path failure, and final-status audit schema/Git-argument failures remain under `outputs/N72R12/**` with traceback or attempt evidence. The last two status-writer issues were repaired without touching experiment artifacts. The existing environment `osr_lib-1.1.0-nspkg.pth` warning is unrelated to the experiment and was not treated as a scientific failure.
- **Decision:** N72R12 is `FAIL_SAFE_INTERVENTION_NOT_SUFFICIENT`. Both structural execution gates pass, but neither deterministic CSI nor the single learned gate beats E0 under the pre-registered global TrackEval and protected-identity criteria. Calibration, selector, decoder LoRA, live requery and production promotion remain unauthorized. Stop gate tuning; the minimum next step is provenance-complete real-human event tape or one newly frozen association-interface probe. Final machine gate: `outputs/N72R12/n72r12_final_gate.json`; controller: `outputs/N72R12/CONTROLLER_STATUS.json`; report: `docs/N72R12_FINAL_REPORT.md`.

## 2026-09-10 — N72R13 temporal intervention value and terminal no-headroom gate

- **Hypothesis/protocol:** N72R13 tested whether a counterfactual `APPLY` chosen by future value could reduce long-horizon identity error. The frozen N72R9 protocol (32 events, 18 independent sequences), N72R11R4 PCTIS checkpoint, exact candidate/solver definitions, and H20/H50/H100 windows were reused. Runtime rows were sealed before GT was opened; the Oracle was explicitly `runtime_eligible=false`, `oracle_upper_bound_only=true`, and all interactions remained `simulated_from_gt` with real-human evidence `0`.
- **Implementation:** Added an explicit `TemporalIdentityState` clone contract, passive BASE future rollout, paired KEEP/APPLY branch isolation, intervention opportunity audit, sequential temporal Oracle, and an independent N72R13 TrackEval exporter/runner/finalizer. The production association formula, Hungarian solver, checkpoint, candidate stream, and `third_party` trees were not changed. The frozen TrackEval checkout stayed at commit `12c8791b303e0a0b50f753af204249e622d0281a`.
- **Opportunity audit:** The frozen 516-interaction corpus contained `30,390` frame rows, `26,072` accepted selections, `3,988` sealed assignment changes, and `3,988` metadata-defined effective opportunities. The formal 32-event sequential Oracle encountered `176` effective opportunities (`RECOVER 82`, `AUTHORITATIVE_REASSIGN 51`, `ATOMIC_ID_SWAP 40`, `ADD_NEW_IDENTITY 3`). Every one selected `KEEP_BASE`; `APPLY_TIV=0`.
- **Execution/failure preservation:** The required RECOVER paired smoke passed after retaining and fixing one scanner false-positive attempt. The first formal Oracle attempt exposed an incorrect GT loader, and the second exposed an incorrect post-hoc scorer import; both full failure artifacts were retained and repaired by routing to the frozen legacy implementations. The third run passed `32/32` events with `0` runtime failures. A CLI `--horizon` mismatch and the initial direct-import failure are also retained. None of these failures changed the scientific protocol.
- **Custom post-hoc result:** Relative to frozen E0, the sequential Oracle showed target identity-error reductions of `+0.116883/+0.052202/+0.013514/+0.009904` at H5/H20/H50/H100 and global identity-error reductions of `+0.016913/+0.006887/+0.002012/+0.002063`. These are not temporal APPLY effects: every effective opportunity chose KEEP, so they reflect the sequential KEEP candidate/state trajectory versus the frozen E0 source and are reported only as a confounded diagnostic.
- **Official interaction-window TrackEval:** The independent exporter and pinned runner passed `288/288` records across H20/H50/H100 for `E0_BASELINE_B0`, frozen `E1B_PCTIS`, and `ORACLE_TIV`. At H100, ORACLE_TIV minus E0 was HOTA `-0.003761`, AssA `-0.002557`, IDF1 `-0.004176`, DetA `-0.004794`, MOTA `-0.009944`, and IDSW reduction `-44` (IDSW increased from `177` to `221`). Thus the required H100 global headroom condition failed.
- **Decision:** `outputs/N72R13/n72r13_final_gate.json` is `STOP_NO_TEMPORAL_INTERVENTION_HEADROOM`; `stage_05_status.json` records no official H100 improvement and `stage_06_status.json` records `NOT_AUTHORIZED_NO_HEADROOM`. No temporal value corpus, value model, calibration head, selector, decoder LoRA, or production promotion was run. `REQUERY_NEXT=true`; the next defensible experiment must isolate the association decision boundary or obtain provenance-complete real-human events, not increase weights/checkpoint capacity or train downstream modules.
- **Evidence:** Stage artifacts are under `outputs/N72R13/`, with final report `docs/N72R13_FINAL_REPORT.md`. Input protocol SHA-256 is `e012ecc3bd64fec4409fccd57d920f3690b8b1c4b5a2dd557e03e2bfb43ef0e9`; PCTIS checkpoint SHA-256 is `77b41dbe2fc0f03c48ca4b8cda22e6b219eb58e266566eb96fcf0df3fb8c2c16`.

## 2026-09-10 — N72R14 persistent public state-conditioned global association

- **Hypothesis/protocol:** N72R14 tested whether persistent public identity state becomes useful when inserted into the full candidate×all-public-ID association matrix. The frozen N72R9 protocol (`32` events, `18` independent sequences), candidate streams, exact public solver, none score, H20/H50/H100 definitions and posthoc scoring were reused. Runtime future-GT use remained false; all events are `simulated_from_gt`; real human tape remains `0`.
- **Implementation:** Added an explicit public-keyed state bank and fixed state edge `r=A+1.0*M+0.5*N-0.1*G`, `Delta=tanh(r)`, `STATE_EDGE_SCALE=1.0`. B0 is corrected E0, P0 adds the target pool, T1 applies only the target public column, and G1 applies the edge to every public column. Event-frame memory reads are false and the first visible state effect is event+1. No SAM3, Hungarian, checkpoint or third-party code was changed.
- **Failure preservation/repair:** The first interface audit had a JSON field-level false non-equivalence and is retained under `outputs/N72R14/interface_audit_failure_attempt_01.json`. The first TrackEval export stopped at a sealed assigned degenerate box in `dancetrack0027` absolute frame `222`/relative frame `74`; exit code `2` and traceback remain under `trackeval/export_failure_attempt_01.json` and `stage_05_attempt_01_failure.json`. Root cause was N72R14's pool builder using `require_positive_geometry=false` instead of the existing corrected pre-solver policy. The minimal fix enabled `require_positive_geometry=true`, then reran a fresh RECOVER smoke and full formal attempt without posthoc clipping/dropping/replacement.
- **Structural result:** Corrected attempt-02 passed the RECOVER smoke, formal `32/32` events, `128` event×variant artifacts and `12,928` runtime rows; assigned invalid geometry was `0`, duplicate/missing keys were `0`, and runtime GT violations were `0`. State propagation passed `3,168/3,168` t→t+1 edge changes with zero unchanged transitions. Pinned official TrackEval passed `384/384` interaction-window records at commit `12c8791b303e0a0b50f753af204249e622d0281a`.
- **Effect:** G1−B0 global identity-error reduction was `+0.000077/-0.004254/-0.005697` at H20/H50/H100 with H100 CI `[-0.014118,+0.013279]`; target H100 reduction was `-0.032973`, with `112` correct versus `216` incorrect target crossings. G1−P0 was `-0.006280/-0.005943/-0.007440`; the H100 TrackEval deltas G1−B0 were HOTA `-0.006968`, AssA `-0.003810`, IDF1 `-0.010685`, IDSW `+30`. RECOVER H100 G1−B0 was `-0.012595`, CI `[-0.063487,+0.011455]`.
- **Decision:** The structural mechanism is connected to the solver, but the strict strong-positive gate is false. The final machine gate is `PERSISTENT_STATE_EDGE_NOT_YET_EFFECTIVE` / `FAIL_FUTURE_EFFECT`; calibration head, selector, decoder LoRA, frequency/strength ablation and human-budget ablation are not authorized. Retain the edge as a negative association-interface diagnostic and use a newly frozen decision-boundary/candidate-quality probe or provenance-complete real-human tape as the next step. Final report: `docs/N72R14_FINAL_REPORT.md`; final gate: `outputs/N72R14/n72r14_final_gate.json`.

## 2026-09-10 — N72R15 repaired persistent identity association

- **Protocol and scope:** Reused the frozen 32-event/18-independent-sequence N72R9/N72R14 protocol, with 4 `ADD_NEW_IDENTITY`, 14 `AUTHORITATIVE_REASSIGN`, 3 `ATOMIC_ID_SWAP`, and 11 `RECOVER_IDENTITY` events. Added a public-keyed persistent state bank and row/column-centered relative edge while keeping the checkpoint, candidate definition, exact solver, metrics, H20/H50/H100 windows, and third-party trees unchanged. All events remain `simulated_from_gt`; real-human evidence remains `0`.
- **Structural result:** Formal attempt 04 passed `32/32` events, `9696` runtime rows, `0` duplicate/missing rows, `0` runtime GT reads, event-frame memory read false, first memory-visible frame event+1, MAIN_B0-only future candidate pool, and `9600/9600` row-max checks. State propagation, candidate coverage, component, self-reinforcement, and assignment-cardinality audits all passed. Pinned TrackEval completed `288/288` interaction-window records at commit `12c8791b303e0a0b50f753af204249e622d0281a`.
- **Effect:** E1J/E1I state edges changed many scores and 3168/3168 checked next edges, but only 2/1 solver-changed frames respectively and no correct E1J-vs-B0 assignment changes. E1J−B0 H100 TrackEval deltas were HOTA `-0.00002760`, AssA `-0.00003097`, IDF1 `-0.00002561`, IDSW `0`; H20 target identity-error reduction was `0` and H100 global CI was `[0,0]`. The strict `FAIL_FUTURE_EFFECT` gate is therefore correct; no ablation, calibration, selector, LoRA, or production promotion was authorized.
- **Mechanism:** Appearance component values were zero/non-discriminative (AUC `0.5`), while E1J relative-edge separability was dominated by motion/native continuity (AUC `0.9596/0.9114`). H100 MAIN_B0 target-visible coverage was `2685/3130=0.857827`, with `333` frames covered by neither diagnostic source. The next defensible step is provenance-complete appearance feature/candidate-quality diagnosis, not larger weights or a new checkpoint.
- **Failure preservation:** Parser, corrected-E0 control reference, `None` public-ID consensus, component schema/O(N²) AUC, event-frame integrity schema, bootstrap seed, TrackEval import, and two finalizer-schema failures were retained under `outputs/N72R15/`; the finalizer ended with `integrity_failures=0` and `FAIL_FUTURE_EFFECT`.
- **Storage cleanup:** Only exact regenerable installer wheels, `__pycache__`, and the project `.pytest_cache` under `/data1/LWR/vranlee/SERVER_ONLY/avis/Interactive/SAM3_InterMOT` were removed, freeing `1,937,313,792` bytes. N16/N20 caches, shared checkpoint, outputs, failure evidence, and all paths outside the current project were preserved. Final report: `docs/N72R15_FINAL_REPORT.md`; final gate: `outputs/N72R15/n72r15_final_gate.json`.
