#!/usr/bin/env python
"""N7 official TrackEval evaluation and statistics for Route A runs."""
...
OUT = Path(os.environ.get("N7_EVAL_OUT", ROOT / "outputs/n7/eval"))
REAL = Path(os.environ.get("N7_REAL_ROOT", ROOT / "outputs/n7/real"))
BUDGETS = [int(x) for x in os.environ.get("N7_BUDGETS", "0 1 2 4 8").split()]
P0_DIR = Path(
    os.environ.get("N7_P0_DIR", ROOT / "outputs/n5/integrity/canonical_mot_results/b0")
)
P3_POST_DIR = Path(
    os.environ.get("N7_P3_POST_DIR", ROOT / "outputs/n6/full25/real/p3_continuous_id_miss")
)
_ALL_SEQS = sorted(
    p.name for p in GT.iterdir() if p.is_dir() and (p / "gt" / "gt.txt").is_file()
)
SEQS = sorted(os.environ.get("N7_SEQS", "").split()) or _ALL_SEQS
FRAME_LIMIT = int(os.environ.get("N7_EVAL_FRAMES", "0")) or None

TRACKERS = ["p0"] + [f"route_a_b{b}" for b in BUDGETS] + ["p3_post"]
    ...
    for name in TRACKERS:
        ...
        for seq in SEQS:
            if name == "p0":
                ...
                src = P3_POST_DIR / seq / "post_mot" / f"{seq}.txt"
            else:
                b = int(name.rsplit("_b", 1)[1])
                src = REAL / f"route_a_b{b}" / seq / "post_mot" / f"{seq}.txt"
...
def main() -> None:
    ...
    qc = [
        {
            "budget": 0 if name == "p0" else (int(name.rsplit("_b", 1)[1]) if name.startswith("route") else None),
            "method": name,
    ...
    pairs = {
        "route_a_b0_vs_p0": ("p0", "p0"),  # B0 is byte-identical to P0
        "route_a_b1_vs_p0": ("route_a_b1", "p0"),
        "route_a_b2_vs_p0": ("route_a_b2", "p0"),
        "route_a_b4_vs_p0": ("route_a_b4", "p0"),
        "route_a_b8_vs_p0": ("route_a_b8", "p0"),
        "p3_post_vs_p0": ("p3_post", "p0"),
    }
    pair_out = {
        name: paired_stats(per_seq_map[a], per_seq_map[b]) for name, (a, b) in pairs.items()
    }
