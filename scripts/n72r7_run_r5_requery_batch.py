#!/usr/bin/env python3
"""Run R5 paired replays serially, one independent CPU process per event."""
...
WORKER = ROOT / "scripts/n72r7_r5_requery_replay.py"
...
def main() -> int:
    ...
    for event_id in args.event_id:
        ...
        print(json.dumps({"event_id": str(event_id), "status": record["status"], "returncode": completed.returncode}, ensure_ascii=False), flush=True)
    status = "PASS_N72R7_R5_REQUERY_REPLAY_BATCH" if not failures else "PARTIAL_N72R7_R5_REQUERY_REPLAY_BATCH"
    atomic_json(output_root / f"batch_attempt{int(args.attempt)}.json", {
        "schema_version": "N72R7_R5_REQUERY_REPLAY_BATCH_V1",
        "status": status,
