#!/usr/bin/env python3
"""Export frozen E0/E1B plus new E1C windows for official TrackEval."""
...
def _runtime_entry(done_path: Path, variant: str) -> tuple[dict[str, Any], Path]:
    done = _read(done_path)
    sealed_path = Path(str(done["runtime_event_sealed"]))
    sealed = _read(sealed_path)
    manifests = sealed.get("runtime_manifests")
    if not isinstance(manifests, dict) or variant not in manifests:
        raise wt.WindowTrackEvalError(f"{done_path}: missing runtime variant {variant}")
    entry = dict(manifests[variant])
    frames = Path(str(entry["frames"]))
    if not frames.is_file() or _sha(frames) != entry.get("frames_sha256"):
        raise wt.WindowTrackEvalError(f"{done_path}: runtime hash mismatch for {variant}")
    if entry.get("runtime_future_gt_used") is not False or entry.get("runtime_gt_read") is not False or entry.get("posthoc_gt_used") is not False:
        raise wt.WindowTrackEvalError(f"{done_path}: runtime GT flag for {variant}")
    return entry, frames


def export(output_root: Path, data_root: Path, r5_manifest_path: Path, safe_manifest_path: Path, trackeval_root: Path) -> dict[str, Any]:
    ...
    if safe.get("status") != "PASS_N72R12_FORMAL_RUNTIME" or safe.get("event_count") != 32:
        raise wt.WindowTrackEvalError("N72R12 safe manifest is incomplete")
    ...
    if len(events) != 32 or set(events) != set(r5_records) or set(events) != set(safe_records):
        raise wt.WindowTrackEvalError("export event key sets are not exactly the frozen 32")
    ...
    for event_id in sorted(events):
        ...
        new_dir = Path(str(safe_records[event_id]["done"])).parent
        entries = {
            ...
            "E1C_PCTIS_SAFE": _runtime_entry(new_dir / "done.json", "E1C_PCTIS_SAFE")[0],
        }
        runtime_rows = {
    ...
    if len(records) != expected or duplicates or missing:
        raise wt.WindowTrackEvalError(f"N72R12 export completeness failed records={len(records)} duplicates={duplicates} missing={missing[:5]}")
    try:
    ...
    manifest = {
        "schema_version": "N72R12_WINDOW_EXPORT_MANIFEST_V1",
        "status": "PASS_EXPORT_N72R12",
        ...
        "records": records,
        "integrity": {"expected_records": expected, "actual_records": len(records), "duplicate_keys": duplicates, "missing_keys": missing, "all_runtime_hashes_verified": True, "all_prediction_files_written_atomically": True},
...
def main() -> int:
    ...
    try:
        manifest = export(args.output_root.resolve(), args.data_root.resolve(), (ROOT / "outputs/N72R11R5R1/formal_e1b_manifest.json"), (ROOT / "outputs/N72R12/formal_safe/formal_safe_manifest.json"), args.trackeval_root.resolve())
        ...
        while (root / f"export_failure_attempt{attempt}.json").exists():
            attempt += 1
        _atomic_json(root / f"export_failure_attempt{attempt}.json", {"schema_version": "N72R12_EXPORT_FAILURE_V1", "status": "FAIL_EXPORT_N72R12", "error_type": type(exc).__name__, "error": str(exc), "traceback": traceback.format_exc(), "historical_outputs_modified": False})
        print(json.dumps({"status": "FAIL_EXPORT_N72R12", "error": str(exc)}, sort_keys=True), file=sys.stderr)
        return 1
