...
def load_past_state_seed_boxes(path: Path, frame_idx: int) -> list[dict[str, Any]]:
    """Load same-sequence runtime boxes from a completed prior window."""
    ...
    with path.open(encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            ...
            if not isinstance(value, dict):
                raise TypeError(f"past-state row is not an object at {path}:{line_no}")
            if int(value.get("frame_idx", -1)) != int(frame_idx):
                continue
            if value.get("box") is None or value.get("public_id") is None:
                raise ValueError(f"past-state seed missing box/public_id at {path}:{line_no}")
            rows.append(value)
    rows.sort(key=lambda value: int(value["public_id"]))
    seen: set[int] = set()
    for row in rows:
        public_id = int(row["public_id"])
        if public_id in seen:
            raise ValueError(f"duplicate past-state public id at frame {frame_idx}: {public_id}")
        seen.add(public_id)
    return rows


def load_past_state_rebind_boxes(path: Path, frame_idx: int) -> list[dict[str, Any]]:
    """Load the latest prior runtime observation for every public track.

    Only rows at or before the new session boundary are eligible.  This is a
    persisted runtime-state snapshot, never a GT or future-frame hint.
    """
    ...
    with path.open(encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            ...
            value = json.loads(line)
            if not isinstance(value, dict):
                raise TypeError(f"past-state rebind row is not an object at {path}:{line_no}")
            if int(value.get("frame_idx", -1)) > int(frame_idx):
                continue
            if value.get("public_id") is None or value.get("box") is None:
                raise ValueError(f"past-state rebind row missing public_id/box at {path}:{line_no}")
            public_id = int(value["public_id"])
            previous = latest.get(public_id)
            if previous is None or int(value["frame_idx"]) > int(previous["frame_idx"]):
                latest[public_id] = value
    rows = [latest[public_id] for public_id in sorted(latest)]
    if not rows:
        raise ValueError(f"no past-state rebind rows at or before frame {frame_idx}: {path}")
    return rows
...
def run_window(
    ...
    output_root: Path,
    gpu: int,
    past_state_seed_path: Path | None = None,
    past_state_rebind_path: Path | None = None,
) -> dict[str, Any]:
    ...
    run_id = f"n72r2-{window_id}-{hashlib.sha256(os.urandom(16)).hexdigest()[:12]}"
    session_id = None
    backend = None
    encoder = None
    ...
    public_track_records: list[dict[str, Any]] = []
    public_matrix_records: list[dict[str, Any]] = []
    exact_public_assignment_pass_count = 0
    exact_public_assignment_failure_count = 0
    seen_frames: set[int] = set()
    raw_keys: set[tuple[int, int]] = set()
    candidate_count = 0
    seed_count = 0
    seed_recovered_count = 0
    seed_failures: list[dict[str, Any]] = []
    rebind_audit: dict[str, Any] | None = None
    manager = None
    track_registry = ActiveTrackAuthority()
    ...
    try:
        ...
        authority_bridge = PublicAuthorityBridge(run_id, sequence, session_id=session_id)
        manager = StateManager(
            StateManagerConfig(variant="reid", score_threshold=-100.0),
            public_authority_resolver=authority_bridge,
        )
        ...
        def write_frame(frame_idx: int, observations: list[Any]) -> None:
            nonlocal candidate_count, exact_public_assignment_pass_count, exact_public_assignment_failure_count
            ...
            frame_records.append({
                ...
                "public_mapping_status": "TRACK_MANAGER_FINAL_MOT_AUTHORITY",
            ...
            # TrackManager is the active MOT authority in this run.  The
            # association state remains a solver-local axis; the explicit
            # bindings below are the only route to a public ID.
            registered_tracks = [
                track_registry.register(frame_idx, obs)
                for obs in observations
            ]
            manager.rollout_frame(frame_idx, manager_obs)
            association_audit = manager.candidate_log[-1]
            assignment = np.asarray(
                association_audit.get("assignment_after_scope", []), dtype=int
            ).reshape(-1)
            state_axis = [
                int(value)
                for value in association_audit.get(
                    "association_state_axis", association_audit.get("public_id_order", [])
                )
            ]
            if assignment.size != len(rows):
                raise RuntimeError(
                    f"association assignment axis mismatch at frame {frame_idx}: "
                    f"{assignment.size} != {len(rows)}"
                )
            for index, state_index in enumerate(assignment.tolist()):
                if state_index < 0 or state_index >= len(state_axis):
                    continue
                track = registered_tracks[index]
                if track is None:
                    raise RuntimeError(
                        f"assigned candidate has no TrackManager authority at "
                        f"{sequence}:{frame_idx}:{index}"
                    )
                authority_bridge.bind_track(
                    frame_idx=frame_idx,
                    candidate_uid=str(rows[index]["candidate_uid"]),
                    association_state_id=state_axis[state_index],
                    track=track,
                    binding_transaction_id=(
                        f"{run_id}:authority:{frame_idx}:{index}:{state_axis[state_index]}"
                    ),
                )
            sidecar = build_assignment_sidecar(
                rows,
                association_audit,
                resolver=authority_bridge,
                source_run_id=run_id,
                session_id=session_id,
            )
            ...
            fused_matrix = np.asarray(
                association_audit.get("fused_scores", []), dtype=np.float64
            )
            if fused_matrix.ndim != 2:
                fused_matrix = np.zeros((len(rows), len(state_axis)), dtype=np.float64)
            public_axis = [
                sidecar_public
                for sidecar_public in sidecar.get("public_id_axis", [])
            ]
            exact_public_assignment: dict[str, Any]
            if len(public_axis) != len(state_axis) or any(value is None for value in public_axis):
                exact_public_assignment = {
                    "schema_version": "N72R2_EXACT_PUBLIC_ASSIGNMENT_V1",
                    "status": "NO_COMPLETE_PUBLIC_AUTHORITY_AXIS",
                    "association_state_axis": state_axis,
                    "public_id_axis": public_axis,
                    "runtime_future_gt_used": False,
                }
                exact_public_assignment_failure_count += 1
            else:
                exact_public_assignment = solve_exact_public_assignment(
                    rows,
                    fused_matrix,
                    state_axis,
                    [int(value) for value in public_axis],
                    none_score=0.0,
                    source_run_id=run_id,
                    session_id=session_id,
                )
                exact_errors = validate_exact_public_assignment(exact_public_assignment)
                if exact_errors:
                    exact_public_assignment["status"] = "FAIL_VALIDATION"
                    exact_public_assignment["validation_errors"] = exact_errors
                    exact_public_assignment_failure_count += 1
                else:
                    exact_public_assignment["status"] = "PASS"
                    exact_public_assignment_pass_count += 1
            public_matrix_records.append(
                {
                    "frame_idx": frame_idx,
                    "candidate_axis": list(sidecar.get("candidate_axis", [])),
                    "public_id_axis": list(sidecar.get("public_id_axis", [])),
                    "candidate_assignment_rows": sidecar.get("candidate_assignment_rows", []),
                    "public_assignment_rows": sidecar.get("public_assignment_rows", []),
                    "none_column_reserved": True,
                    "none_column_solver_enabled": exact_public_assignment.get("status") == "PASS",
                    "none_score": 0.0,
                    "solver_source": "N72R2_exact_public_assignment_module; StateManager_matrix_preserved",
                    "exact_public_assignment": exact_public_assignment,
                    "runtime_future_gt_used": False,
                }
            )
            for obs_index, obs in enumerate(observations):
                track = registered_tracks[obs_index]
                if track is None:
                    continue
                public_track_records.append(
                    {
                        ...
                        "frame_idx": frame_idx,
                        "candidate_uid": next(
                            row["candidate_uid"] for row in rows
                            if int(row["adapter_external_id"]) == int(obs.sam_object_id)
                        ),
                        "raw_sam_id": None if obs.raw_sam_object_id is None else int(obs.raw_sam_object_id),
                        "adapter_external_id": int(obs.sam_object_id),
                        "mot_track_id": int(track.mot_track_id),
                        "public_id": int(track.mot_track_id),
                        "lineage_id": int(track.identity_lineage_id),
                        "box": np.asarray(obs.box_xyxy, dtype=float).tolist(),
                        "feature": np.asarray(
                            rows[obs_index].get("feature") or [], dtype=float
                        ).tolist(),
                        "authority_source": "TrackManager.final_mot_track_id",
                        "runtime_future_gt_used": False,
                    }
                )
        ...
        if past_state_rebind_path is not None and past_state_seed_path is not None:
            raise ValueError("past-state rebind and per-object seed modes are mutually exclusive")
        if past_state_rebind_path is not None:
            seed_rows = load_past_state_rebind_boxes(past_state_rebind_path, frame_start)
            rebind_result = backend.rebind_past_state_boxes(
                frame_start,
                [
                    (
                        100000 + int(seed_row["public_id"]),
                        np.asarray(seed_row["box"], dtype=float),
                    )
            ...
            rebind_audit = {
                key: value
                for key, value in rebind_result.items()
                if key != "observations"
            }
            rebind_audit["failures"] = [
                {
                    **failure,
                    "public_id": int(failure["object_id"]) - 100000,
                }
                for failure in rebind_result["failures"]
            ]
            seed_count = len(seed_rows)
            seed_recovered_count = int(rebind_result["recovered_count"])
            seed_failures = [
                {
                    **failure,
                    "public_id": int(failure["object_id"]) - 100000,
            ...
            initial = backend.get_frame_outputs(frame_start)
        elif past_state_seed_path is not None:
            initial = backend.detect_concept(frame_start, "person")
            seed_rows = load_past_state_seed_boxes(past_state_seed_path, frame_start)
            for seed_row in seed_rows:
                seeded = backend.seed_box_from_past_state(
                    frame_start,
                    100000 + int(seed_row["public_id"]),
                    np.asarray(seed_row["box"], dtype=float),
                )
                if seeded is None:
                    seed_failures.append(
                        {
                            "public_id": int(seed_row["public_id"]),
                            "seed_object_id": 100000 + int(seed_row["public_id"]),
                            "reason": "official_sam3_returned_no_observation_for_past_state_box",
                        }
                    )
                else:
                    seed_recovered_count += 1
            if seed_failures and seed_recovered_count == 0:
                # A failed past-state prompt must not leave the official
                # session with an empty prompt/cache.  Re-establish the
                # original concept prompt on the same frame so this recovery
                # attempt still emits honest candidate-recall evidence.
                initial = backend.detect_concept(frame_start, "person")
            else:
                initial = backend.get_frame_outputs(frame_start)
            # The last official prompt response contains the complete
            # recovered seed set.  The source is past runtime state, not a
            # simulated human action or a GT-derived observation.
            seed_count = len(seed_rows)
        else:
            initial = backend.detect_concept(frame_start, "person")
        ...
        public_tracks_path = output_root / "public_mot_tracks.jsonl"
        public_matrix_path = output_root / "public_assignment_matrix.jsonl"
        authority_bindings_path = output_root / "public_authority_bindings.jsonl"
        equivalence_audit = {
        ...
        assigned_rows = [
            ...
            for item in record["sidecar"].get("candidate_assignment_rows", [])
            if item.get("association_state_id") is not None
        ]
        mapped_rows = [item for item in assigned_rows if item.get("public_id") is not None]
        public_authority_audit = {
            "schema_version": "N72R2_PUBLIC_AUTHORITY_AUDIT_V1",
            "status": (
                "PASS_TRACK_MANAGER_FINAL_MOT_AUTHORITY"
                if assigned_rows and len(mapped_rows) == len(assigned_rows)
                else "PARTIAL_PUBLIC_AUTHORITY_MAPPING"
            ),
            ...
            "association_assignment_row_count": len(assigned_rows),
            "public_assignment_row_count": len(mapped_rows),
            "unmapped_assignment_row_count": len(assigned_rows) - len(mapped_rows),
            "mapping_coverage": (len(mapped_rows) / len(assigned_rows)) if assigned_rows else 1.0,
            "public_assignment_artifact_absent_count": sum(
                len(item["sidecar"].get("public_assignment_rows", []))
                for item in sidecar_records
            ),
            "track_manager_authority_binding_count": len(authority_bridge.bindings),
            "public_id_is_association_state_id": False,
            "binding_source": "TrackManager.final_mot_track_id",
            "runtime_future_gt_used": False,
        }
        ...
        atomic_jsonl(public_tracks_path, public_track_records)
        atomic_jsonl(public_matrix_path, public_matrix_records)
        atomic_jsonl(
            authority_bindings_path,
            [
                {**binding.as_dict(), "runtime_future_gt_used": False}
                for binding in authority_bridge.bindings
            ],
        )
        atomic_json(equivalence_path, equivalence_audit)
        atomic_json(public_authority_path, public_authority_audit)
        seed_input_present = past_state_seed_path is not None or past_state_rebind_path is not None
        seed_complete = (not seed_input_present) or not seed_failures
        done = {
            "schema_version": "N72R2_STAGE01_GPU_SMOKE_DONE_V1",
            "status": (
                "PASS_CANDIDATE_V2_SAME_RUN_PUBLIC_AUTHORITY"
                if public_authority_audit["status"].startswith("PASS") and seed_complete
                else "PARTIAL_PUBLIC_AUTHORITY_MAPPING"
            ),
            "sequence": sequence,
            "window_id": window_id,
            ...
            "sidecar_public_authority_present": bool(mapped_rows),
            "public_mapping_status": public_authority_audit["status"],
            "public_authority_audit": public_authority_audit,
            "public_track_row_count": len(public_track_records),
            "public_assignment_matrix_frame_count": len(public_matrix_records),
            "exact_public_assignment_pass_frame_count": exact_public_assignment_pass_count,
            "exact_public_assignment_failure_frame_count": exact_public_assignment_failure_count,
            "local_global_audit": ledger.audit(),
            "checkpoint_sha256": metadata["checkpoint_sha256"],
            "runtime_config_sha256": metadata["runtime_config_sha256"],
            "runtime_memory_policy": backend.runtime_memory_policy(),
            "past_state_seed_count": seed_count,
            "past_state_seed_recovered_count": seed_recovered_count,
            "past_state_seed_failure_count": len(seed_failures),
            "past_state_seed_failures": seed_failures,
            "past_state_seed_complete": seed_complete,
            "past_state_seed_source": None if past_state_seed_path is None else str(past_state_seed_path),
            "past_state_rebind_source": None if past_state_rebind_path is None else str(past_state_rebind_path),
            "past_state_rebind_audit": rebind_audit,
            "runtime_future_gt_used": False,
            "process_isolation": "one_python_process_one_sam3_session_one_frame_range",
            "outputs": {"legacy_candidates": str(legacy_path), "candidate_v2": str(candidate_path), "candidate_frames": str(frame_path), "assignment_sidecar": str(sidecar_path), "mapping_ledger": str(mapping_path), "equivalence_audit": str(equivalence_path), "public_authority_audit": str(public_authority_path), "public_tracks": str(public_tracks_path), "public_assignment_matrix": str(public_matrix_path), "authority_bindings": str(authority_bindings_path)},
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=N72R2_ROOT / "outputs/N72R2/bridge/stage_01_smoke")
    parser.add_argument("--status-path", type=Path, default=None, help="stage status destination; used by multi-window wrapper")
    parser.add_argument("--past-state-seed", type=Path, default=None, help="same-sequence runtime public tracks used only to recover an independent session")
    parser.add_argument("--past-state-rebind", type=Path, default=None, help="same-sequence prior runtime tracks for one official multi-box rebind")
    args = parser.parse_args()
    output_root = args.output_root.resolve()
    status_path = (args.status_path.resolve() if args.status_path is not None else N72R2_ROOT / "outputs/N72R2/stage_01_status.json")
    ...
    try:
        ...
        result = run_window(
            ...
            output_root,
            int(args.gpu),
            args.past_state_seed,
            args.past_state_rebind,
        )
        status = {
            "schema_version": "N72R2_STAGE_STATUS_V1",
            "stage": "01_PUBLIC_AUTHORITY_BRIDGE",
            "status": "PASS_PUBLIC_AUTHORITY_BRIDGED" if result["status"].startswith("PASS") else "PARTIAL_PUBLIC_AUTHORITY_MAPPING",
            "candidate_export": result,
            "research_efficacy": "NOT_RUN",
            "real_human_event_count": 0,
            "runtime_future_gt_used": False,
            "public_id_inferred": False,
            "next_minimum_action": "Run the frozen multi-window handover gate; public IDs are authoritative only through TrackManager.final_mot_track_id.",
            "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        }
        atomic_json(status_path, status)
        atomic_json(output_root / "stage_01_status.json", status)
        ...
        failure = {
            ...
            "real_human_event_count": 0,
            "public_id_inferred": False,
            "next_minimum_action": "Preserve this failure and perform only the N72R2-approved targeted retry on the same frozen window.",
        }
