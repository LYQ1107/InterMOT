#!/usr/bin/env bash
# Round 2: abstention + guard selection on top of R1-CA.
set -euo pipefail

PROJECT=.
PY="$PROJECT/envs/sam3_intermot/bin/python"
ROOT_OUT="$PROJECT/outputs/n4/round2"
LOG_ROOT="$ROOT_OUT/logs"
mkdir -p "$LOG_ROOT"

declare -a JOBS
GPU_LIST=(1 2 3)
for cfg in R2_G1 R2_G2 R2_G3 R2_G4; do
  for b in 1 2 5; do
